...
class dlc_table_maker:
    ...
    def get_dlc_details (self):
        if os.path.exists('dlc.json'):
            ...
            dlc_details = {}
        while 'game_type' not in dlc_details.keys() or not dlc_details['game_type'] in ['kuro', 'sky', 'xanadu', 'ys_x']:
            raw_game_type = str(input("Game Type: [kuro, sky, xanadu, ys_x] ")).encode('utf-8').decode('utf-8').lower()[:6]
            if raw_game_type in ['kuro', 'sky', 'xanadu', 'ys_x']:
        ...
        if self.game_type == 'xanadu':
            while 'dlc_desc2' not in dlc_details.keys() or dlc_details['dlc_desc'] == '':
                dlc_details['dlc_desc2'] = str(input("DLC Description 2: ")).encode('utf-8').decode('utf-8')
                dlc_details['dlc_desc2'] = dlc_details['dlc_desc'].replace('\\n','\n') # Allow newlines
        ...
        if self.game_type in ['kuro', 'sky', 'xanadu']:
    ...
    def get_chr_id (self, mdl_name):
        try:
            if self.game_type in ['kuro', 'sky', 'xanadu']:
                ...
                if not base_name in self.name_dict:
                    if base_name + '_c10' in self.name_dict: # Kuro 2
                        base_name = base_name + '_c10'
                    elif base_name + '_c15' in self.name_dict: # Kai
                        base_name = base_name + '_c15'
                    else: # Weird catch-all, e.g. Fie in Kai - _c10/_c15 gets priority though
                        costume_matches = [x for x in self.name_dict if x[:len(base_name)] == base_name]
                        if len(costume_matches) > 0:
                            base_name = costume_matches[0]
            elif self.game_type == 'ys_x':
    ...
    def get_mdl_details (self):
        ...
        for i in range(len(models)):
            ...
            if self.game_type == 'kuro':
                ...
                while 'category' not in mdl_details.keys() or mdl_details['category'] not in [15,16,17,19]:
                    print("Item type: [15=costume, 16=hair color, 17=accessory, 19=Orbment cover, leave blank for 15]")
                    item_type_raw = input("Item type for {0}: ".format(models[i]))
                    if item_type_raw == '':
                        print("No entry given, setting item type to default of 15.")
                        mdl_details['category'] = 15
                    else:
                        try:
                            mdl_details['category'] = int(item_type_raw)
                            if mdl_details['category'] not in [15,16,17,19]:
                                print("Invalid entry!")
                        except ValueError:
                            print("Invalid entry!")
                if 'subcategory' not in mdl_details.keys() or mdl_details['subcategory'] not in [16,17]:
                    mdl_details['subcategory'] = {15:16, 16:16, 17:16, 19:17}[mdl_details['category']]
                while 'attach_name' not in mdl_details.keys()\
                        or (mdl_details['attach_name'] == '' and mdl_details['category'] == 17):
                    if mdl_details['category'] == 17:
                        mdl_details['attach_name'] = str(input("Attach point for {0}: (e.g. Head_Point - check model .json file for valid options)  ".format(models[i]))).encode('utf-8').decode('utf-8')
                    else:
                        mdl_details['attach_name'] = ''
                if 'attach_txt1' not in mdl_details.keys():
                    mdl_details['attach_txt1'] = {15:'', 16:'', 17:'hold', 19:''}[mdl_details['category']]
            elif self.game_type == 'xanadu':
                while 'category' not in mdl_details.keys() or mdl_details['category'] not in [19,30]:
                    print("Item type: [19=costume, hair color, accessory; 30=Xiphone wallpaper, leave blank for 19]")
                    item_type_raw = input("Item type for {0}: ".format(models[i]))
                    if item_type_raw == '':
                        print("No entry given, setting item type to default of 19.")
                        mdl_details['category'] = 19
                    else:
                        try:
                            mdl_details['category'] = int(item_type_raw)
                            if mdl_details['category'] not in [19,30]:
                                print("Invalid entry!")
                        except ValueError:
                            print("Invalid entry!")
                while 'subcategory' not in mdl_details.keys() or mdl_details['subcategory'] not in [35,36,37,40]:
                    if mdl_details['category'] == 19:
                        print("Item subtype: [35=costume, 36=hair color, 37=accessory, leave blank for 35]")
                        item_type_raw = input("Item type for {0}: ".format(models[i]))
                        if item_type_raw == '':
                            print("No entry given, setting item type to default of 35.")
                            mdl_details['subcategory'] = 35
                        else:
                            try:
                                mdl_details['subcategory'] = int(item_type_raw)
                                if mdl_details['subcategory'] not in [35,36,37]:
                                    print("Invalid entry!")
                            except ValueError:
                                print("Invalid entry!")
                    else:
                        mdl_details['subcategory'] = 40 # Xiphone wallpaper
                if 'icon' not in mdl_details.keys():
                    mdl_details['icon'] = {35:93, 36:95, 37:94, 40:76}[mdl_details['subcategory']]
                while 'attach_name' not in mdl_details.keys()\
                        or (mdl_details['attach_name'] == '' and mdl_details['subcategory'] == 37):
                    if mdl_details['subcategory'] == 37:
                        mdl_details['attach_name'] = str(input("Attach point for {0}: (e.g. Hat_Point - check model .json file for valid options)  ".format(models[i]))).encode('utf-8').decode('utf-8')
                    else:
                        mdl_details['attach_name'] = ''
                if 'attach_txt1' not in mdl_details.keys():
                    mdl_details['attach_txt1'] = {35:'', 36:'', 37:'hold', 40:''}[mdl_details['subcategory']]
            elif self.game_type == 'ys_x':
            ...
            if self.game_type == 'xanadu' and 'name2' not in mdl_details.keys():
                mdl_details['name2'] = ''
            ...
            while 'stores' not in mdl_details.keys():
                ...
                if store_opt == 'y':
                    if (os.path.exists('t_shop.tbl') or os.path.exists('t_shop.tbl.original')
                        or os.path.exists('t_shop_normal.tbl') or os.path.exists('t_shop_normal.tbl.original')):
                        if os.path.exists('t_shop.tbl'):
                            shop_info = self.kt.read_table('t_shop.tbl')['ShopInfo']
                        elif os.path.exists('t_shop.tbl'):
                            shop_info = self.kt.read_table('t_shop.tbl.original')['ShopInfo']
                        elif os.path.exists('t_shop_normal.tbl'):
                            shop_info = self.kt.read_table('t_shop_normal.tbl')['ShopNormalInfo']
                        else: # t_shop_normal.tbl.original
                            shop_info = self.kt.read_table('t_shop_normal.tbl.original')['ShopNormalInfo']
                        valid_stores = [x['id'] for x in shop_info]
                        need_list = input("Would you like a list of stores? (y/N) ")
                        if not need_list == '':
                            need_list = need_list[0].lower()
                        if need_list == 'y':
                        ...
                        print("t_shop.tbl / t_shop_normal.tbl unavailable, cannot display shop names.")
            ...
            if self.game_type == 'kuro':
                ...
                if 'effect_icon' not in mdl_details.keys() or mdl_details['effect_icon'] == '':
                    mdl_details['effect_icon'] = 0
                if 'element' not in mdl_details.keys() or mdl_details['element'] == '':
                    mdl_details['element'] = 0
                if 'int1' not in mdl_details.keys() or mdl_details['int1'] == '':
                    mdl_details['int1'] = 0
                if 'float1' not in mdl_details.keys() or mdl_details['float1'] == '':
                    mdl_details['float1'] = 0
                if 'float2' not in mdl_details.keys() or mdl_details['float2'] == '':
                    mdl_details['float2'] = 0
                if 'eff1_id' not in mdl_details.keys() or mdl_details['eff1_id'] == '':
                    mdl_details['eff1_id'] = 0
                if 'eff1_0' not in mdl_details.keys() or mdl_details['eff1_0'] == '':
                    mdl_details['eff1_0'] = 0
                ...
                if 'eva' not in mdl_details.keys() or mdl_details['eva'] == '':
                    mdl_details['eva'] = 0
                if 'aev' not in mdl_details.keys() or mdl_details['aev'] == '':
                    mdl_details['aev'] = 0
                if 'crit' not in mdl_details.keys() or mdl_details['crit'] == '':
                    mdl_details['crit'] = 0
                if 'spd' not in mdl_details.keys() or mdl_details['spd'] == '':
                ...
                if 'flags' not in mdl_details.keys():
                    mdl_details['flags'] = ''
                if 'flags2' not in mdl_details.keys():
                    mdl_details['flags2'] = ''
                if 'unk0' not in mdl_details.keys() or mdl_details['unk0'] == '':
                    mdl_details['unk0'] = 0
                if 'unk1' not in mdl_details.keys() or mdl_details['unk1'] == '':
                ...
                if 'eff5_2' not in mdl_details.keys() or mdl_details['eff5_2'] == '':
                    mdl_details['eff5_2'] = 0
                if 'itm_lvl' not in mdl_details.keys() or mdl_details['itm_lvl'] == '':
                    mdl_details['itm_lvl'] = 0
                if 'stack_size' not in mdl_details.keys() or mdl_details['stack_size'] == '':
                    mdl_details['stack_size'] = {35:1, 36:1, 37:5, 40:1}[mdl_details['subcategory']]
                if 'price' not in mdl_details.keys() or mdl_details['price'] == '':
                    mdl_details['price'] = 100
                if 'sort_id' not in mdl_details.keys() or mdl_details['sort_id'] == '':
                    mdl_details['sort_id'] = mdl_details['id']
                if 'unk2' not in mdl_details.keys() or mdl_details['unk2'] == '':
                    mdl_details['unk2'] = 0
                if 'unk3' not in mdl_details.keys() or mdl_details['unk3'] == '':
                    mdl_details['unk3'] = 0
                ...
                if 'attach_txt0' not in mdl_details.keys():
                    mdl_details['attach_txt0'] = ''
                if 'mdl_variant' not in mdl_details.keys():
                    mdl_details['mdl_variant'] = ''
                if 'attach_unk1' not in mdl_details.keys() or mdl_details['attach_unk1'] == '':
                    mdl_details['attach_unk1'] = 0
                if 'attach_unk2' not in mdl_details.keys() or mdl_details['attach_unk2'] == '':
                    mdl_details['attach_unk2'] = 0
                if 'attach_txt1' not in mdl_details.keys():
                    mdl_details['attach_txt1'] = ''
                if 'attach_txt2' not in mdl_details.keys():
                    mdl_details['attach_txt2'] = ''
            elif self.game_type == 'ys_x':
    ...
    def make_item_entry (self, mdl_name):
        ...
        if self.game_type == 'kuro':
            ...
            return({"id": mdl_data['id'], "chr_restrict": mdl_data['chr_restrict'], "flag1": mdl_data['flags'],
                ...
                "icon": mdl_data['icon'], "unk0": mdl_data['unk0'], "unk1": mdl_data['unk1'], "eff1_id": mdl_data['eff1_id'],
                "eff1_0": mdl_data['eff1_0'], "eff1_1": mdl_data['eff1_1'], "eff1_2": mdl_data['eff1_2'],
                "eff2_id": mdl_data['eff2_id'], "eff2_0": mdl_data['eff2_0'], "eff2_1": mdl_data['eff2_1'],
                "eff2_2": mdl_data['eff2_2'], "eff3_id": mdl_data['eff3_id'], "eff3_0": mdl_data['eff3_0'],
                "eff3_1": mdl_data['eff3_1'], "eff3_2": mdl_data['eff3_2'], "eff4_id": mdl_data['eff4_id'],
                "eff4_0": mdl_data['eff4_0'], "eff4_1": mdl_data['eff4_1'], "eff4_2": mdl_data['eff4_2'],
                "eff5_id": mdl_data['eff5_id'], "eff5_0": mdl_data['eff5_0'], "eff5_1": mdl_data['eff5_1'],
                "eff5_2": mdl_data['eff5_2'], "itm_lvl": mdl_data['itm_lvl'], "stack_size": mdl_data['stack_size'],
                "price": mdl_data['price'], "sort_id": mdl_data['sort_id'], "unk2": mdl_data['unk2'],
                "unk3": mdl_data['unk3'], "unk4": mdl_data['unk4'], "anim": mdl_data['anim'],
                "name": mdl_data['name'], "name2": mdl_data['name2'], "desc": mdl_data['desc']})
        elif self.game_type == 'ys_x':
    ...
    def make_costume_entry (self, mdl_name):
        ...
        if self.game_type == 'kuro':
            return({"char_restrict": mdl_data['chr_restrict'], "type": {17:0, 18:2, 19:1, 24:3}[mdl_data['category']],
                ...
                "mdl_name": mdl_name.lower().split('.mdl')[0], "unk1": mdl_data['attach_unk1'], "unk2": mdl_data['attach_unk2'],
                "attach_name": mdl_data['attach_name'], "unk_txt1": mdl_data['attach_txt1'], "unk_txt2": mdl_data['attach_txt2']})
        elif self.game_type == 'sky':
            return({"char_restrict": mdl_data['chr_restrict'], "type": {15:0, 16:2, 17:1, 19:3}[mdl_data['category']],
                "item_id": mdl_data['id'], "unk0": mdl_data['attach_unk0'], "unk_txt0": mdl_data['attach_txt0'],
                "mdl_name": mdl_name.lower().split('.mdl')[0], "unk1": mdl_data['attach_unk1'], "unk2": mdl_data['attach_unk2'],
                "attach_name": mdl_data['attach_name'], "unk_txt1": mdl_data['attach_txt1'], "unk_txt2": mdl_data['attach_txt2']})
        elif self.game_type == 'xanadu':
            return({"char_restrict": mdl_data['chr_restrict'], "type": {35:0, 36:2, 37:1, 40:0}[mdl_data['subcategory']],
                "item_id": mdl_data['id'], "unk0": mdl_data['attach_unk0'], "unk_txt0": mdl_data['attach_txt0'],
                "mdl_name": mdl_name.lower().split('.mdl')[0], "mdl_variant": mdl_data['mdl_variant'],
                "unk1": mdl_data['attach_unk1'], "unk2": mdl_data['attach_unk2'], "attach_name": mdl_data['attach_name'],
                "unk_txt1": mdl_data['attach_txt1'], "unk_txt2": mdl_data['attach_txt2']})
        elif self.game_type == 'ys_x':
    ...
    def make_dlc_entry (self):
        ...
        if self.game_type in ['kuro', 'sky']:
            ...
            return({"id": self.dlc_details['id'], "sort_id": self.dlc_details['sort_id'], "items": items, "unk0": self.dlc_details['unk0'], "quantity": quant, "unk1": self.dlc_details['unk1'], "name": self.dlc_details['dlc_name'], "desc": self.dlc_details['dlc_desc'], "desc2": self.dlc_details['dlc_desc2'], "unk_txt": self.dlc_details['unk_txt']})
        elif self.game_type == 'ys_x':
        ...
        if self.game_type in ['kuro', 'sky', 'xanadu']:
            for i in range(len(mdl_data['stores'])):
                shop_entries.append({"shop_id": mdl_data['stores'][i], "item_id": mdl_data['id'],
                    "unknown": {'kuro':1, 'sky':1, 'xanadu':0}[self.game_type],
                    "start_scena_flags": [], "empty1": 0, "end_scena_flags": [], "int2": 0})
        elif self.game_type == 'ys_x':
            for i in range(len(mdl_data['stores'])):
    ...
    def make_recipe_entries (self, mdl_name):
        ...
        recipe_entries = []
        if self.game_type in ['kuro', 'sky', 'xanadu']:
            pass # Not used for Kuro / Sky / Kyoto Xanadu
        elif self.game_type == 'ys_x':
    ...
    def make_item_tbl_data (self):
        ...
        for i in range(len(self.model_list)):
            self.kurodlc_json[subtable_header].append(self.make_item_entry(self.model_list[i]))
        return

    def make_costume_tbl_data (self):
        subtable_header = 'CostumeTable' if self.game_type == 'ys_x' else 'CostumeParam'
        self.kurodlc_json[subtable_header] = []
        for i in range(len(self.model_list)):
    ...
    def make_dlc_tbl_data (self):
        subtable_header = 'DLCTable' if self.game_type == 'ys_x' else 'DLCTableData'
        self.kurodlc_json[subtable_header] = [self.make_dlc_entry()]
        return

    def make_shop_tbl_data (self):
        subtable_header = {'kuro': 'ShopItem', 'sky': 'ShopItem',
            'xanadu': 'ShopNormalItem', 'ys_x': 'ProductInfo'}[self.game_type]
        self.kurodlc_json[subtable_header] = []
        for i in range(len(self.model_list)):
