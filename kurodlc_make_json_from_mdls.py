...
class dlc_table_maker:
    ...
    def get_dlc_details (self):
        if os.path.exists('dlc.json'):
            ...
            dlc_details = {}
        while 'game_type' not in dlc_details.keys() or not dlc_details['game_type'] in ['kuro', 'sky', 'ys_x']:
            raw_game_type = str(input("Game Type: [kuro, sky, ys_x] ")).encode('utf-8').decode('utf-8').lower()[:4]
            if raw_game_type in ['kuro', 'sky', 'ys_x']:
    ...
    def make_recipe_entries (self, mdl_name):
        mdl_data = self.models[mdl_name]
        recipe_entries = []
        if self.game_typein ['kuro', 'sky']:
            pass # Not used for Kuro / Sky
        elif self.game_type == 'ys_x':
    ...
    def make_costume_tbl_data (self):
        subtable_header = {'kuro': 'CostumeParam', 'sky': 'CostumeParam', 'ys_x': 'CostumeTable'}[self.game_type]
    ...
    def make_dlc_tbl_data (self):
        subtable_header = {'kuro': 'DLCTableData', 'sky': 'DLCTableData', 'ys_x': 'DLCTable'}[self.game_type]
        self.kurodlc_json[subtable_header] = [self.make_dlc_entry()]
        return

    def make_shop_tbl_data (self):
        subtable_header = {'kuro': 'ShopItem', 'sky': 'ShopItem', 'ys_x': 'ProductInfo'}[self.game_type]
        self.kurodlc_json[subtable_header] = []
        for i in range(len(self.model_list)):
